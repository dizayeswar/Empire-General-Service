...
Deno.serve(async (req) => {
  ...
  if (req.method === "GET") {
    return json({ ok: true, msg: "Empire API running (Supabase)", version: "2026-09-21-mohammed-lawyer" });
  }
  ...
  try {
    ...
    switch (action) {
      ...
      case "markApplicationIssueSeen": return json(await misc.handleMarkApplicationIssueSeen(body, a));
      case "deleteApplicationIssue": return json(await misc.handleDeleteApplicationIssue(body, a));
      case "clearApplicationIssues": return json(await misc.handleClearApplicationIssues(body, a));
      ...
      case "markWarehouseSapDone": return json(await warehouse.handleMarkWarehouseSapDone(body, a));
      ...
      case "saveWarehouseGinReceivedSig": return json(await warehouse.handleSaveWarehouseGinReceivedSig(body, a));
      case "getWarehouseLayout": return json(await warehouse.handleGetWarehouseLayout(body));
      case "saveWarehouseLayout": return json(await warehouse.handleSaveWarehouseLayout(body, a));
      case "getWarehouseSignatures": return json(await warehouse.handleGetWarehouseSignatures(body));
      case "saveWarehouseSignatures": return json(await warehouse.handleSaveWarehouseSignatures(body, a));
